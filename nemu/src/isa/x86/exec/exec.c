...
static inline def_EHelper(gp3) {
  switch (s->isa.ext_opcode) {
    IDEXW(0, test_I, test, -1) EMPTY(1) EXW(2, not,-1) EMPTY(3)
    EXW(4,mul,-1) EXW(5, imul1,-1) EMPTY(6) EXW(7, idiv,-1)
  }
}
...
again:
  ...
  switch (opcode) {
    ...
    IDEXW (0x18, G2E, adc, 1)
    IDEX (0x19, G2E, adc)
    IDEXW (0x1a, E2G, adc, 1)
    IDEX (0x1b, E2G, adc)
    IDEXW (0x1c, I2a, adc, 1)
    IDEX (0x1d, I2a, adc)
    //and
